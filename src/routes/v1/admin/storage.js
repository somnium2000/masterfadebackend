...
import { buildAssetReadUrl, deleteAsset, prepareStorageUpload } from "../../../services/storage/storageService.js";

const ADMIN_STORAGE_ROLES = ["super_admin", "admin"];
...
export default async function adminStorageRoutes(app) {
  app.post(
    "/uploads/prepare",
    {
      preHandler: app.requireRoles(ADMIN_STORAGE_ROLES),
      schema: {
        body: {
          type: "object",
          properties: {
  ...
  app.delete(
    ...
    async (request, reply) => {
      try {
        const deleted = await deleteAsset(app, {
          claims: request.claims,
          assetId: request.params.id,
        });
        ...
        return sendHandled(
          ...
          request,
          error,
          "No se pudo eliminar asset de Storage",
          "ADMIN_STORAGE_DELETE_ERROR"
        );
      }
    }
  );
